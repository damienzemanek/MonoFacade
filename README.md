...
`UnboundFunctionality` for modules that are not invoked via an event
`BoundFunctionality` for modules invoked via an event
`BoundSetFunctionality` for modules that are invoked via an event and receive data
6. Unity Player Loop compatability with `TickFM` execution channels routed through functionality modules
7. Optional `FSM` Integration: Easily create States with pre-existing Functionailty modules just by adding `FSM_ON_ENTER` or `FSM_ON_EXIT` interaface labels
8. Easily create transitions between FSM modules
